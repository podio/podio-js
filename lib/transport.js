...
module.exports = {
  ...
  _onTokenRefreshed: function(request) {
    if (request.requestType === 'generic') {
      this.request(request.method, request.path, request.data);
    } else if (request.requestType === 'file') {
      this.uploadFile(request.filePath, request.fileName);
    }
  },
  ...
  _handleTransportError: function(options, response) {
    ...
    switch(response.status) {
      case 400:
        error = new PodioErrors.PodioBadRequestError(response.body, response.status, request.url);
        break;
      case 401:
        this._getAuth()._clearAuthentication();
        error = new PodioErrors.PodioAuthorizationError(response.body, response.status, request.url);
        break;
      case 403:
        error = new PodioErrors.PodioForbiddenError(response.body, response.status, request.url);
        break;
      case 404:
        error = new PodioErrors.PodioNotFoundError(response.body, response.status, request.url);
        break;
      case 409:
        error = new PodioErrors.PodioConflictError(response.body, response.status, request.url);
        break;
      case 410:
        error = new PodioErrors.PodioGoneError(response.body, response.status, request.url);
        break;
      case 420:
        error = new PodioErrors.PodioRateLimitError(response.body, response.status, request.url);
        break;
      case 500:
        error = new PodioErrors.PodioServerError(response.body, response.status, request.url);
        break;
      case 502:
      case 503:
      case 504:
        error = new PodioErrors.PodioUnavailableError(response.body, response.status, request.url);
        break;
      default:
        error = new PodioErrors.PodioError(response.body, response.status, request.url);
        break;
    }
