(function (root, factory) {
  ...
  var AuthLib = {
    ...
    _authRequest: function(url, requestData, callback) {
      request.post(url).type('form').send(requestData).end(this._onAuthResponse.bind(this, callback, requestData.grant_type));
    },
    ...
    _onAuthResponse: function(callback, grantType, res) {
      if (res.ok) {
        ...
        throw new Error('Authentication for ' + grantType + ' failed. Reason: ' + res.body.error_description);
      }
    }
  };
