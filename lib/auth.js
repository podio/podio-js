...
module.exports = {
  ...
  configureTrustedConnection: function(path, requestData, callback) {
    var url = new URI(this.apiURL).path(path).toString();
    var auth = 'Basic ' + new Buffer(this.clientId + ':' + this.clientSecret).toString('base64');

    request.post(url).set('Authorization', auth).send(requestData).end(function (res) {
      if (res.ok) {
        callback(res.body);
      } else {
        throw new PodioErrors.PodioError('Request failed. Reason: ' + res.body.error_description, res.status, url);
      }
    });
  },
